...
export default function Dashboard() {
  ...
  return (
      <div className="relative min-h-screen w-full">
        ...
        {/* 2. READABILITY OVERLAY (Vignette) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPlayers.map((p) => (
                <Link
                  key={p.id}
                  to={`/players/${p.id}`}
                  className="group relative bg-gray-900/40 backdrop-blur-md border border-white/10 rounded-2xl p-6 transition-all hover:bg-white/[0.08] hover:border-blue-500/50 hover:-translate-y-1 shadow-xl overflow-hidden"
                >
                  ...
                  <div className="absolute -right-4 -top-4 text-white/[0.02] font-black text-6xl italic select-none">
                    {p.position || "P"}
                  </div>
